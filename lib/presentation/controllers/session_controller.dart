...
class SessionController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    _setupBleListeners();
    _setupCollarBootTimeListener();
  }

  /// FIX 4: Setup collar boot time listener for Phase-1 safety
  void _setupCollarBootTimeListener() {
    _bleService.onCollarBootTimeCalculated = ({
      required int collarBootTimeUtcMs,
      required int firstPacketCollarTs,
      required int firstPacketMobileUtc,
    }) async {
      // Only create annotation if we have an active session
      if (currentSession.value == null) return;

      print('[Session] 🕐 Recording collar boot time annotation');

      try {
        final now = DateTime.now();
        final elapsedMs = sessionStartTime.value != null
            ? now.difference(sessionStartTime.value!).inMilliseconds
            : 0;

        await _annotationRepo.createAnnotation(
          sessionId: currentSession.value!.id,
          timestampUtc: now.toUtc(),
          elapsedMs: elapsedMs,
          phase: currentPhase,
          category: AnnotationCategory.system,
          type: 'collar_time_sync',
          description: 'Collar boot time synchronized',
          severity: AnnotationSeverity.info,
          structuredData: {
            'collar_boot_time_utc_ms': collarBootTimeUtcMs,
            'first_packet_collar_ts_ms': firstPacketCollarTs,
            'first_packet_mobile_utc_ms': firstPacketMobileUtc,
          },
          annotatorUserId: observerId.isNotEmpty ? observerId : 'SYSTEM',
          isAutoGenerated: true,
        );

        print('[Session] ✅ Collar boot time annotation created');
      } catch (e) {
        print('[Session] ⚠️ Failed to create collar boot time annotation: $e');
      }
    };
  }
    ...
    try {
      ...
      try {
        ...
        await _sessionRepo.updatePhase(
          ...
          collarTimestampMs: collarTimestamp > 0 ? collarTimestamp : null,
        );
      } catch (e) {
      ...
      try {
        ...
        // FIX 1: Create permanent record of mode transition for Phase-1 safety
        final now = DateTime.now();
        final elapsedMs = sessionStartTime.value != null
            ? now.difference(sessionStartTime.value!).inMilliseconds
            : 0;
        ...
        await _annotationRepo.createAnnotation(
          ...
          timestampUtc: now.toUtc(),
          elapsedMs: elapsedMs,
          phase: SessionPhase.surgery,
          category: AnnotationCategory.system,
          type: 'mode_transition_to_raw',
          description: 'Collar switched to RAW mode for surgery',
          severity: AnnotationSeverity.info,
          structuredData: {
            'collar_confirm_ms': now.millisecondsSinceEpoch,
            'mobile_utc_ms': now.millisecondsSinceEpoch,
            'collar_mode': 'raw',
            'firmware_ack': true,
            'target_mode': 'raw',
            'previous_mode': 'filtered',
          },
          annotatorUserId: observerId.isNotEmpty ? observerId : 'SYSTEM',
          isAutoGenerated: true,
        );

        print('[Session] 📝 Mode transition annotation created for Phase-1 safety');
      } catch (e) {

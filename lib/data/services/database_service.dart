...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> updateSessionPhase(String id, String phase) async {
    await (update(localSessions)..where((s) => s.id.equals(id))).write(
      LocalSessionsCompanion(
        ...
        surgeryStartedAt: phase == 'surgery'
            ? Value(DateTime.now())
            : const Value.absent(),
        surgeryEndedAt: phase == 'recovery'
            ? Value(DateTime.now())
            : const Value.absent(),
        endedAt: phase == 'completed'
            ? Value(DateTime.now())
            : const Value.absent(),
  ...
  Future<void> deleteOldSyncedVitals({int keepDays = 7}) async {
    ...
    await (delete(localVitals)..where(
          (v) =>
              v.syncStatus.equals('synced') &
              v.createdAt.isSmallerThanValue(cutoff),
        ))
        .go();
  }
  ...
  Future<int> getSyncQueueCount() async {
    final count = await (selectOnly(
      syncQueue,
    )..addColumns([syncQueue.id.count()])).getSingle();
  ...
  Future<List<CachedAnimal>> searchCachedAnimals(String query) async {
    ...
    return (select(cachedAnimals)
          ..where(
            (a) =>
                a.name.lower().like(lowerQuery) |
                a.ownerName.lower().like(lowerQuery),
          )
          ..limit(20))
        .get();
  }
  ...
  Future<void> clearOldCache({int keepDays = 7}) async {
    ...
    await (delete(
      cachedAnimals,
    )..where((a) => a.cachedAt.isSmallerThanValue(cutoff))).go();
  }
}
